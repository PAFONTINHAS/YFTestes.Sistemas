...
function abrirModal(row) {
    ...
    xhr.open("GET", "../Gestao/obterDadosModal/obterDadosDespesa.php?id=" + id, true);
    xhr.send();
}
    ...
    if (confirm("Tem certeza que deseja pagar essa despesa?")) {
        ...
        if (dataPagamentoInput === "") {
            ...
            // Executar a lógica de pagamento da despesa
                var xhr = new XMLHttpRequest();
                xhr.open('POST', '../../classes/Despesa.php', true);
                ...
                var params = 'id=' + idDespesa + '&dataPagamento=' + dataPagamentoInput + "&id_usuario=" + id_usuario;
                xhr.send(params);
...
function excluirDespesa() {
    if (confirm("Tem certeza que deseja excluir essa despesa?")) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '../../classes/Despesa.php', true); // Alterado para 'pagarDespesa.php'
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                // A requisição foi concluída com sucesso
        ...
        xhr.send('idExcluir=' + idDespesa + "&id_usuario=" + id_usuario);
        fecharModalDP();
    }
}
