function abrirModal(row) {
    ...
    var modalReceitaRecebida = document.getElementById("modalReceitaRecebida");
    ...
    modalReceitaRecebida.setAttribute("data-id", id);
    ...
    xhr.open("GET", "../Gestao/obterDadosModal/obterDadosReceita.php?id=" + id, true);
    xhr.send();
}
    ...
    if (confirm("Tem certeza que deseja receber essa receita?")) {
        ...
        if (dataRecebimento === "") {
            ...
            // Executar a lógica de pagamento da receita
                var xhr = new XMLHttpRequest();
                xhr.open('POST', '../../classes/Receita.php', true);
                ...
                var params = 'id=' + idReceita + '&dataRecebimento=' + dataRecebimento + '&id_usuario=' + id_usuario;
                xhr.send(params);
...
function excluirReceita() {
    if (confirm("Tem certeza que deseja excluir essa receita?")) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '../../classes/Receita.php', true); // Alterado para 'pagarReceita.php'
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                // A requisição foi concluída com sucesso
        ...
        xhr.send('idExcluir=' + idReceita + '&id_usuario=' + id_usuario);
        fecharModalReceitaR();
